...
def extract_gcode_from_3mf(input_3mf, output_gcode):
    """Extract G-code from a 3MF file - copied from ReplaceBaseplate."""
...
def find_gcode_3mf_file(stl_path):
    ...
    if os.path.exists(potential_gcode_3mf):
        print(f"📁 Found G-code 3MF file: {potential_gcode_3mf}")
    ...
    for file in os.listdir(stl_dir):
        if file.endswith('.gcode.3mf'):
            ...
            print(f"📁 Found G-code 3MF file: {found_gcode_3mf}")
...
def extract_gcode_from_3mf_file(three_mf_path):
    ...
    print(f"📦 Extracting gcode from: {three_mf_path}")
    ...
    try:
        ...
        print("✅ Gcode extracted successfully")
    ...
    for i, line in enumerate(lines):
        ...
        if m and 'Z' in m:
            ...
            if first_z is None:
                first_z = z_val
                first_layer_start = i
                print(f"📊 Found first layer at Z={z_val}, starting at line {i}")
            elif abs(z_val - first_z) > 0.1:  # Significant Z change indicates new layer
                first_layer_end = i
                print(f"📊 First layer ends at line {i} (Z changed from {first_z} to {z_val})")
                break
...
def parse_gcode_first_layer(gcode_content):
    ...
    if layer_ranges:
        ...
        print(f"📊 Found first layer {first_layer_idx} at lines {first_layer_start}-{first_layer_end}")
    else:
        # Fallback: find first layer by Z movements
        print("⚠️  No ;LAYER: comments found, using Z-based layer detection...")
    ...
    if not pts:
        print("⚠️  No extrusion points found, trying fallback...")
    ...
    print(f"📊 Found {len(pts)} points in first layer")
    return np.array(pts)
...
def compute_convex_hull(points):
    ...
    print(f"✅ Convex hull computed with {len(hull_points)} vertices")
...
def calculate_hull_metrics(hull_points):
    ...
    print("📏 Calculating hull metrics...")
...
def main():
    ...
    try:
        ...
        if input_file.endswith('.gcode.3mf'):
            ...
            print(f"📁 Using provided G-code 3MF file: {gcode_3mf_path}")
        else:
