...
def create_stl_from_hull(hull_points, height=1.0, output_path="extruded_hull.stl"):
    ...
    """
    print(f"🔺 Creating STL from convex hull with {len(hull_points)} vertices...")
    print(f"📏 Extrusion height: {height}mm")
    ...
    print(f"📊 Extruded volume: {volume:.2f} mm³")
    print(f"📊 Surface area: {extruded_mesh.areas.sum():.2f} mm²")
...
def load_hull_from_file(file_path):
    ...
    print(f"📁 Loaded {len(points)} hull points from {file_path}")
    return points
...
def main():
    ...
    if not os.path.exists(args.hull_file):
        print(f"❌ Error: Hull file not found: {args.hull_file}")
        sys.exit(1)
    ...
    try:
        ...
        print(f"\n✅ Successfully created extruded STL: {args.output}")
